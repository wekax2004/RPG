...
(() => {
  ...
  var AssetManager = class {
    ...
    init() {
      ...
      this.images[124] = this.createTool("shovel");
      this.images[65] = this.createTool("rope");
      this.images[43] = this.createTool("machete");
      this.images[66] = this.createTool("pickaxe");
      this.images[203] = this.createGem("#f44336");
      this.images[204] = this.createGem("#2196f3");
      this.images[172] = this.createGem("#eee");
      this.images[86] = this.createPotion();
      this.images[2] = this.createArmorPlate("#b0bec5", "#78909c");
      ...
      this.images[301] = this.createGeneric("#fff59d");
      this.images[302] = this.createFence();
      this.images[305] = this.createGeneric("#8d6e63");
      this.images[306] = this.createGeneric("#5d4037");
      this.images[125] = this.createHole();
      this.images[126] = this.createHole();
      this.images[140] = this.createClub("#8d6e63", false);
      this.images[9] = this.createNPC("#4caf50", "#5d4037");
      this.images[252] = this.createNPC("#8bc34a", "#795548");
      this.images[253] = this.createNPC("#1b5e20", "#3e2723");
      this.images[251] = this.createNPC("#ffcc80", "#607d8b");
      this.images[254] = this.createNPC("#ffcc80", "#795548");
      this.images[255] = this.createNPC("#ffcc80", "#9c27b0");
  ...
  var ItemRegistry = {
    ...
    30: { name: "Parcel", type: "container", stackable: false, uIndex: 33 },
    // Changed to 33
    ...
    140: { name: "Wooden Club", type: "weapon", attack: 8, slot: "rhand", uIndex: 140 },
    // Fixed uIndex 140
  ...
  function toolSystem(world2, input2, ui2) {
    if (input2.isJustPressed("MouseLeft")) {
      ...
      if (uiAny.targetingItem) {
        ...
        if (!game2 || !game2.map) return;
        const tile = game2.map.getTile(tx, ty);
        const player2 = world2.query([PlayerControllable2, Position])[0];
        if (player2) {
          const pPos = world2.getComponent(player2, Position);
          const dx = pPos.x - wx;
  ...
  function spawnMapItem(x, y, id) {
    ...
    const tile = game2.map.getTile(tileX, tileY);
    if (tile) {
  ...
  function loadGame(world2) {
    const json = localStorage.getItem(SAVE_KEY);
    if (!json) return false;
    try {
      const data = JSON.parse(json);
      const pEnt = world2.query([PlayerControllable2])[0];
      if (pEnt === void 0) return false;
      ...
      if (inv) {
        ...
        data.player.inventory.equipment.forEach((e) => {
          ...
          if (itemDef) {
            inv.equip(e.slot, new ItemInstance2(itemDef, e.count));
          }
        });
        let bagDef = Object.values(ItemRegistry).find((i) => i.name === "Backpack");
        if (!bagDef) {
          bagDef = ItemRegistry[0];
        }
        if (bagDef) {
          const bagInst = new ItemInstance2(bagDef, 1);
          bagInst.contents = [];
          data.player.inventory.backpack.forEach((b) => {
            const iDef = ItemRegistry[b.itemIdx];
            if (iDef) {
              bagInst.contents.push(new ItemInstance2(iDef, b.count));
            }
          });
  ...
  async function start() {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      if (mobName.includes("Dragon")) {
        lootItems.push(createItemFromRegistry(SPRITES.GOLDEN_HELMET));
      }
      if (Math.random() < 0.1) {
    ...
    if (backpackItem) {
      ...
      inv.addItem(createItemFromRegistry(SPRITES.SHOVEL));
      inv.addItem(createItemFromRegistry(SPRITES.ROPE));
    }
  ...
  var game = {
    update: (dt) => {
      if (world && input) {
        ...
        if (player && player.id !== void 0) {
          const pEnt = player.id;
